...
class YoPhoneValidator
{
    ...
    {
        if (YoPhoneValidator::validate($msisdn) === false) {
            throw new Exception(
                'The supplied MSISDN is not valid. ' .
                'You can use the `YoPhoneValidator::validate()` method ' .
                'to validate the MSISDN being passed.',
                400
            );
        }
        $this->msisdn = YoPhoneValidator::clean($msisdn);
    }
    ...
    {
        $mobileNumber = YoPhoneValidator::clean($mobileNumber);
